...
- Calcular sugerencias combinando artistas similares (vía artist.getInfo).
...
class LastFMClient:
    ...
    # ---------- BÚSQUEDA POR TEXTO (artist.search) ----------
...
# =========================
# API pública: MB + Discogs (con fallback MBID → nombre)
# =========================
...
# =========================
# Lógica de sugerencias (usando getInfo)
# =========================
...
def main():
    ...
    st.title("🎧 Explorador de artistas (Last.fm) + discos de estudio (MB / Discogs)")
    ...
    if st.session_state.last_suggestions_time is not None:
        st.info(
            f"⏱ Último cálculo de sugerencias: "
    ...
    st.markdown(
        "1. Escribe un nombre de artista y añádelo a la lista.\n"
        "2. Verás hasta **10 artistas similares**, usando `artist.getInfo`.\n"
        "3. Para cada artista seleccionado, puedes ver sus **discos de estudio** "
        "con rating de vinilo desde Discogs (cuando esté disponible).\n"
    )
                ...
                with st.container():
                    ...
                    if st.button("📀 Ver discos de estudio", key=f"albums_{idx}"):
                        ...
                        if df_albums.empty:
                            ...
                            st.success(
                                f"⏱ Discos de estudio para **{artist.name}**: "
